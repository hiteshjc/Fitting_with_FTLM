...
/////////////////////////////////////////////////
class IMatrix : public std::vector<int>
{
  size_t m;
  size_t n;
...
public:
   IMatrix() : m(0), n(0) {}
   IMatrix(size_t m_, size_t n_) : std::vector<int>(m_*n_), m(m_), n(n_) {}
   IMatrix(size_t m_, size_t n_, int val) : std::vector<int>(m_*n_, val), m(m_), n(n_) {}

  void resize(size_t m_, size_t n_)
    {m = m_; n = n_; std::vector<int>::resize(m_*n_);}
  void reserve(size_t m_, size_t n_)
    {std::vector<int>::reserve(m_*n_);}
  void clear()
    {m = n = 0; std::vector<int>::clear();}
