...
void calc_hints_xyyzzx(double coupling, 
                   ...
                   std::vector< complex<double> > &hints_list)
{
       	int a=btest(spin_det,first); //x 
        int b=btest(spin_det,second); //y
       	int c=btest(spin_det,third);  //y
       	int d=btest(spin_det,fourth); //z
       	int e=btest(spin_det,fifth); //z
       	int f=btest(spin_det,sixth); //x
	double sign1=1.0;
        double sign2=1.0; 
	...
	if (c==1) sign2=-1.0;
        ...
        // XYZXYZ - only one out of 16 terms survive
	double signsz=(double(d)-0.5)*(double(e)-0.5);
        hints_list.push_back( (-64.0/16.0) * coupling*signsz*sign1*sign2);

	int new_spin_det=spin_det;
	new_spin_det=ibclr(new_spin_det,first);
	new_spin_det=ibclr(new_spin_det,second);
	new_spin_det=ibclr(new_spin_det,third);
	new_spin_det=ibclr(new_spin_det,sixth);
	
	if (a==0) new_spin_det=ibset(new_spin_det,first);
	if (b==0) new_spin_det=ibset(new_spin_det,second);
	if (c==0) new_spin_det=ibset(new_spin_det,third);
	if (f==0) new_spin_det=ibset(new_spin_det,sixth);
