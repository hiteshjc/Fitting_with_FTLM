...
class Spin_Half_Ross: public Ham
{
    ...
    void init(std::vector< std::vector<int> > &filepairs, std::vector<int> & sublattice, 
	      double J1, double J2, double J3, double J4, double gxy, double gy, double gz, double hx, double hy, double hz)
    {
	this->bondtype.clear();
	this->pairs.clear();
	...
	make_g_mats_with_h(gxy,gy,gz,this->hx,this->hy,this->hz,this->gmats);
    }
	...
	this->k3=k3;
	make_J_mats(J1,J2,J3,J4,this->Jmats);
	make_g_mats_with_h(gxy,gy,gz,this->hx,this->hy,this->hz,this->gmats); // hx, hy, hz are in meV
    }
